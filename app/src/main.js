...
const App = {
    ...
    trackOrgan: function() {
        const organID = document.getElementById('inputOrganID').value;
        
        // Assuming you have a mapping or method in your smart contract to check organ status
        App.contracts.OrganDonation.deployed().then(function(instance) {
            return instance.getOrganStatus(organID);  // Assuming the smart contract has this function
        }).then(function(result) {
            const status = result;  // E.g., 'In Transit', 'Delivered', etc.
            document.getElementById('organStatus').innerText = "Organ Status: " + status;
        }).catch(function(err) {
            console.log(err.message);
            document.getElementById('organStatus').innerText = "Error fetching status";
        });
    },   
    addOrgan: async function(organID, organType, donorAddress) {
        try {
            await this.contractInstance.methods.addOrgan(organID, organType, donorAddress).send({ from: this.accounts[0] });
            console.log('Organ added successfully');
        } catch (error) {
            console.error('Error adding organ:', error);
        }
    },
    updateOrganStatus: async function(organID, status) {
        try {
            await this.contractInstance.methods.updateOrganStatus(organID, status).send({ from: this.accounts[0] });
            console.log('Organ status updated successfully');
        } catch (error) {
            console.error('Error updating organ status:', error);
        }
    },
    getOrganStatus: async function(organID) {
        try {
            const status = await this.contractInstance.methods.getOrganStatus(organID).call();
            console.log('Organ status:', status);
            return status;
        } catch (error) {
